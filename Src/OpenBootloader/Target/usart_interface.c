...
static void OPENBL_USART_Init(void)
{
  ...
  USART_InitStruct.Parity              = LL_USART_PARITY_NONE;      // OpenOSD-X NONE
...
//    while( 0x7f != OPENBL_USART_ReadByte());
...
uint8_t OPENBL_USART_ReadByte(void)
{
  ...
  {
    OPENBL_IWDG_Refresh();
    if ( READ_BIT(USARTx->ISR, USART_ISR_IDLE|USART_ISR_ORE|USART_ISR_FE|USART_ISR_PE) ) {
        WRITE_REG(USARTx->ICR, USART_ISR_IDLE|USART_ISR_ORE|USART_ISR_FE|USART_ISR_PE);
    }
  }
